...
class PostApi {
  Future<http.Response> createPost(
      {required String title, required String description, required List<XFile> files}) async {
    var request = http.MultipartRequest('POST', Uri.parse(AppStrings.apiUrl + '/posts'))
  ...
  Future<http.Response> getDetailedPost(String uuid) async {
    var response = await http.get(Uri.parse(AppStrings.apiUrl + '/posts/' + uuid), headers: HttpHeaders.baseHeaders);
    return response;
  }

  Future<http.Response> likePost(String uuid) async {
    var response =
        await http.post(Uri.parse('${AppStrings.apiUrl}/posts/$uuid/like'), headers: HttpHeaders.baseHeaders);
    return response;
  }

  Future<http.Response> removeLikePost(String uuid) async {
    var response =
        await http.delete(Uri.parse('${AppStrings.apiUrl}/posts/$uuid/like'), headers: HttpHeaders.baseHeaders);
    return response;
  }

  Future<http.Response> getUsersPost(String uuid) async {
    var response =
        await http.get(Uri.parse(AppStrings.apiUrl + '/posts?user_uuid=' + uuid), headers: HttpHeaders.baseHeaders);
    return response;
  }
